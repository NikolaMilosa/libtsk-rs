use crate::{bindings as tsk, errors::TskError, tsk_fs_file::TskFsFile};
...
impl<'a, 'f, 'fs> NonResidentData<'a, 'f, 'fs> {
    ...
    ///
    pub fn new(_tsk_fs_attr: &'a TskFsAttr<'f, 'fs>, nrd: tsk::TSK_FS_ATTR__bindgen_ty_1) -> Self {
        Self { _tsk_fs_attr, nrd }
    }
    ...
    pub fn run<'dr>(&'dr self) -> TskFsAttrRun<'dr, 'a, 'f, 'fs> {
        TskFsAttrRun::from_nrd(&self, self.nrd.run)
    }
...
impl<'f, 'fs> TskFsAttr<'f, 'fs> {
    ...
    pub fn from_index(
        ...
        let tsk_fs_attr = unsafe {
            tsk::tsk_fs_file_attr_get_idx(tsk_fs_file.into(), tsk_fs_file_attr_get_idx as _)
        };
        ...
        if tsk_fs_attr.is_null() {
            ...
            let error_msg_ptr = unsafe { NonNull::new(tsk::tsk_error_get() as _) }.ok_or(
                TskError::tsk_attr_error(format!(
            ...
            return Err(TskError::tsk_attr_error(format!(
                "There was an error getting the TskFsAttr at index {}: {}",
    ...
    /// Create a TSK_FS_ATTR wrapper given the TskFsFile and index of the attribute
    pub fn from_default(tsk_fs_file: &'f TskFsFile<'fs>) -> Result<Self, TskError> {
        // Get a pointer to the TSK_FS_ATTR sturct
        let tsk_fs_attr = unsafe { tsk::tsk_fs_file_attr_get(tsk_fs_file.into()) };
    ...
    pub fn get_non_resident_data<'a>(&'a self) -> Option<NonResidentData<'a, 'f, 'fs>> {
        if (self.attr_flags() & tsk::TSK_FS_ATTR_FLAG_ENUM_TSK_FS_ATTR_NONRES) > 0 {
            unsafe { Some(NonResidentData::new(&self, (*self.tsk_fs_attr).nrd)) }
        } else {
            None
        }
    }
...
impl<'fs, 'f> Read for TskFsAttr<'fs, 'f> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        let attr_size: usize = self.size().try_into().unwrap();
        ...
        self._offset += TryInto::<i64>::try_into(bytes_read).unwrap();
