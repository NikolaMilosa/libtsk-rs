...
unsafe extern "C" fn img_info(_arg1: *mut tsk::TSK_IMG_INFO, _arg2: *mut tsk::FILE) {
    trace!("img_info() not implemented");
}

unsafe extern "C" fn img_close(_arg1: *mut tsk::TSK_IMG_INFO) {
...
impl TskImgReadSeek {
    pub fn from_read_seek<S: Into<String>>(
        ...
        // Set the stream
        unsafe {
            std::ptr::addr_of_mut!((*boxed_tsk_reader.as_mut_ptr()).stream).write(Box::new(stream))
        };
        // Get the pointer to the uninitialized struct
        let tsk_reader_ptr_unint: *mut MaybeUninit<TskImgReadSeekInner> =
            Box::into_raw(boxed_tsk_reader).cast();
        ...
        let close = Some(img_close as unsafe extern "C" fn(arg1: *mut tsk::TSK_IMG_INFO));
        let imgstat = Some(
            img_info as unsafe extern "C" fn(arg1: *mut tsk::TSK_IMG_INFO, arg2: *mut tsk::FILE),
        );

        // Get address of the TSK_IMG_INFO pointer
        let img_addr =
            unsafe { std::ptr::addr_of_mut!((*(*tsk_reader_ptr_unint).as_mut_ptr()).tsk_img_info) };
        // Open via the external source
        let tsk_img_info_ptr: *mut tsk::TSK_IMG_INFO = unsafe {
            tsk::tsk_img_open_external(img_addr as _, size, sector_size, read, close, imgstat)
        };
