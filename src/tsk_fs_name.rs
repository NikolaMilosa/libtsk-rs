use crate::{bindings as tsk, errors::TskError};
use std::ffi::CStr;
use std::ptr::NonNull;
    ...
    pub fn from_ptr(tsk_fs_name: *const tsk::TSK_FS_NAME) -> Result<Self, TskError> {
        if tsk_fs_name.is_null() {
            ...
            let error_msg_ptr = unsafe { NonNull::new(tsk::tsk_error_get() as _) }.ok_or(
                TskError::lib_tsk_error(format!("Error TSK_FS_NAME is null. (no context)")),
            )?;
            ...
            return Err(TskError::tsk_fs_name_error(format!(
                "Error TSK_FS_NAME is null: {}",
                error_msg
            )));
        }
