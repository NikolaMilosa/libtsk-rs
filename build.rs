...
fn windows_compile_tsk() {
    ...
    let program_path =
        PathBuf::from(std::env::var("ProgramFiles(x86)").expect("NO ProgramFiles(x86) ENV."));
    // Now we need to compile the vswhere.exe path
    let vs_where_path =
        PathBuf::from(&program_path).join(r"Microsoft Visual Studio\Installer\vswhere.exe");
    // Run vswhere.exe to get install path
    let output = Command::new(&vs_where_path)
        .args(&["-latest", "-property", "installationPath"])
        .output()
        .expect(&format!(
            r"Error executing vswhere: {}",
            vs_where_path.display()
        ));

    // Get the installation location
    let install_path = PathBuf::from(String::from_utf8_lossy(&output.stdout).trim_end());
    ...
    let libtsk_vcxproj_contents =
        fs::read_to_string(r"sleuthkit\win32\libtsk\libtsk.vcxproj").unwrap();
...
fn windows_setup() {
    windows_compile_tsk();
    println!(
        r"cargo:rustc-link-search={}\sleuthkit\win32\x64\Release_NoLibs",
        env::var("CARGO_MANIFEST_DIR").unwrap()
    );

    let hklm = RegKey::predef(HKEY_LOCAL_MACHINE);
    let sdk_key = hklm
        .open_subkey(r"SOFTWARE\Wow6432Node\Microsoft\Microsoft SDKs\Windows\v10.0")
    ...
    let product_version: String = sdk_key
